...
def answer_question_loc(query, collection, top_n, prefix: str | None = None):
    ...
    prompt = (
        ...
        "请根据以下标准内容用自然语言回答我的问题，如果相关内容不存在则说明查无结果：\n\n"
        f"{context}\n\n"
        f"我的问题是：{query}\n\n"
        "（回答问题，并说明来源-文件（可以是多个）"
        "如果文本内容与文本完全无关，只回答：地方标准中查无结果"
    )
    messages = [
        {"role": "system", "content": "你是文件检索机器人，擅长从给出的文本出分析相关问题答案"},
        {"role": "user",   "content": prompt},
    ]
    return get_completion(messages)

def answer_question_cn(query, collection, top_n, prefix: str | None = None):
    embedding = get_embeddings([query])[0]
    results = collection.query(
        query_embeddings=[embedding],
        n_results=top_n,
    ...
    prompt = (
        ...
        f"我的问题是：{query}\n\n"
        "（回答问题，并说明来源-文件（可以是多个）"
        "如果问题是地方性质的，则查找在国家标准中有没有与问题有关的资料（比如询问当地用水标准，可以查找是否有国家用水标准），如果文本内容与文本完全无关只回答：国家标准中查无结果"
    )
    messages = [
        {"role": "system", "content": "你是文件检索机器人，擅长从给出的文本出分析相关问题答案"},
        {"role": "user",   "content": prompt},
...
def answer_question(query):
    ...
    coll_loc = client.get_or_create_collection("coll-loc")   # 地方标准
    coll_cn  = client.get_or_create_collection("coll_cn")    # 国家标准
    ...
    # 情况①：地方 + 国家（6 + 4）
    if std_type == "loc":
        ctx_loc = answer_question_loc(query, coll_loc, top_n=6, prefix="地方标准")
        ctx_cn  = answer_question_cn(query, coll_cn, top_n=4, prefix="国家标准")
        return ctx_loc + "\n\n" + ctx_cn

    # 情况②：只返回国家标准（10）
    return answer_question_cn(query, coll_cn, top_n=10, prefix="国家标准")
